...
document.addEventListener('DOMContentLoaded', function() {
    ...
    parseBtn.addEventListener('click', function() {
        ...
        if (allFunctions.length === 0) {
            alert('No methods found in the code!');
            return;
        }
    ...
    function handleFile(file) {
        ...
        reader.onload = function(e) {
            ...
            javaCodeInput.value = currentFileContent;
            ...
            successMsg.innerHTML = `
                <div style="display: flex; flex-direction: column; gap: 5px;">
                    ...
                    <div style="margin-left: 32px; display: flex; flex-direction: column; gap: 3px; font-size: 0.9rem;">
                        ...
                        <div><strong>Type:</strong> ${file.type || 'Java Source File'}</div>
    ...
    // Parse Java code to extract methods
    function parseFunctions(code) {
        const functions = [];
        const lines = code.split('\n');
        
        // Enhanced regex to match Java method declarations with indentation
        // Matches: [whitespace] [annotations] [modifiers] returnType methodName(
        // Handles: public/private/protected, static, final, abstract, synchronized, generics, arrays
        const functionRegex = /^\s*(?:@\w+(?:\([^)]*\))?\s+)*(?:public\s+|private\s+|protected\s+)?(?:static\s+)?(?:final\s+)?(?:abstract\s+)?(?:synchronized\s+)?(?:native\s+)?(?:\w+(?:<[^>]+>)?(?:\[\])*\s+)(\w+)\s*\(/gm;
    ...
    function displayFunctionList(functions) {
        functionCount.textContent = `${functions.length} method${functions.length !== 1 ? 's' : ''} found`;
    ...
    function displayFunction(func) {
        ...
        const linesHtml = codeLines.map((line, index) => {
            ...
            const highlightedLine = line ? Prism.highlight(line, Prism.languages.java, 'java') : ' ';
            return `<span class="code-line" data-line="${lineNumber}">${highlightedLine}</span>`;
        }).join('\n');
        
        codeDisplay.innerHTML = `<pre><code class="language-java">${linesHtml}</code></pre>`;
